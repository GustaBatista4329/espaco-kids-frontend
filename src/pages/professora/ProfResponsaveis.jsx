...
export function ProfResponsaveis({ onNavigate }) {
  const { api } = useAuth();
  ...
  return (
    <div>
      ...
      {loading ? (
        ...
        <EmptyState icon={Users} title="Nenhum responsável" subtitle="Nenhum responsável cadastrado ainda" />
      ) : (
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: 12 }}>
          {filtered.map((r) => (
            <Card key={r.id} hover onClick={() => onNavigate("prof-ver-alunos", { responsavelId: r.id, responsavelNome: r.nome })}>
